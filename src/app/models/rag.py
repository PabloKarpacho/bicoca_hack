...
class SearchPayload(BaseModel):
    file_id: str | None = Field(
        default=None,
        description="Document or file identifier associated with the payload item.",
    )
    page_content: str | None = Field(
        default=None, description="Extracted page or chunk text content."
    )
    page_number: int | None = Field(
        default=None,
        description="Page number associated with the payload item, when available.",
    )
    file_description: str | None = Field(
        default=None,
        description="Optional human-readable description of the source file.",
    )


class SearchResponse(BaseModel):
    payload: SearchPayload = Field(
        description="Payload returned for the search result item."
    )
